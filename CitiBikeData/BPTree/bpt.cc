...
int bplus_tree::sortOrderAscending(value_t *values, size_t max, bool *next) const
    {
        off_t off_left = meta.leaf_offset;
        off_t off = off_left;
        size_t i = 0;
        record_t *b, *e;
        leaf_node_t leaf;
    ...
    int bplus_tree::sortOrderDescending(value_t *values, size_t max, bool *next) const
    {
        off_t off_firstLeaf = meta.leaf_offset;
        off_t off_lastLeaf = lastLeafOffset();
        
        
        
        off_t current_off = off_lastLeaf;
            size_t i = 0;
            record_t *b, *e;
        leaf_node_t leaf;
        while (current_off != 0 && i < max) {
            map(&leaf, current_off);
            b = begin(leaf) - 1;
            e = leaf.children + leaf.n - 1;
            for (; b != e && i < max; --e, ++i)
                values[i] = e->value;
            current_off = leaf.prev;
        }
        for (i; i < max; i++) {
            values[i] = -1;
        }
...
off_t bplus_tree::lastLeafOffset() const
{
    off_t org = meta.root_offset;
    int height = meta.height;
    while (height > 1) {
        ...
        index_t *i = end(node) - 1;
        org = i->child;
        --height;
    }
    
    internal_node_t lastInternal;
    map(&lastInternal, org);
    index_t *i = end(lastInternal) - 1;
    off_t lastLeafOffset = i->child;
